...
To create a random account and add the `DEPLOYER_PRIVATE_KEY_ENCRYPTED` to the `.env` file, run:
...
If you prefer to manually set your own private key, you can use the `yarn account:import` command to encrypt and store your private key. You will get prompted to set a encryption password before storing your `DEPLOYER_PRIVATE_KEY_ENCRYPTED` to `packages/hardhat/.env`.
