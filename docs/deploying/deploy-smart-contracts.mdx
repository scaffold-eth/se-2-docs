...
By default `yarn deploy` run deployment scripts from `packages/foundry/script` folder and deploy to local network. You can change `defaultNetwork` in `packages/foundry/foundry.toml` file.
