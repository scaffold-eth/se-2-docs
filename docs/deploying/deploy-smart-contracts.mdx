...
It will automatically generate a new [keystore](https://book.getfoundry.sh/reference/cast/cast-wallet-import) with the name `scaffold-eth-custom`.
...
To deploy to a specific network, you can use the `--network` flag.
...
By default, `yarn deploy` runs deployment scripts from the `packages/foundry/script` folder and deploys to the local network. You can change the `defaultNetwork` in the `packages/foundry/foundry.toml` file. To deploy to a specific network, use the `--network` flag:
