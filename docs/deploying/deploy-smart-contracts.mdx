...
By default, `pnpm run deploy` will deploy the contract to the local network. You can change the defaultNetwork in `packages/hardhat/hardhat.config.ts.` You could also simply run `pnpm run deploy --network target_network` to deploy to another network.
...
```
pnpm run deploy --network network_name
```
