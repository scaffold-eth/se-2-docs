...
<details>
<summary>How to add a chain to the frontend using viem/chains</summary>

To add a new chain to your frontend:

1. Import the chain from `viem/chains` in your `scaffold.config.ts` file.
2. Add the imported chain to the `targetNetworks` array.

Here's an example of how to add the Optimism network:

```ts
import { hardhat, optimism } from "viem/chains";

export const scaffoldConfig = {
  targetNetworks: [hardhat, optimism],
