...
If you need to interact with external contracts you can use `wagmi` directly, or you can add external contract data to your `externalContracts.ts` file, which would let you use Scaffold-ETH 2 hooks with autocompletion to interact with them.
...
<details>
<summary>Mock test example of `externalContracts.ts` file adding some popular contracts in different networks:</summary>

```ts title="contracts/externalContracts.ts"
