...
<details open>
<summary>Here is how you can add a chain that already exists in viem/chains:</summary>

[viem/chains](https://viem.sh/docs/chains/introduction) is an entrypoint for popular EVM-compatible chains. Most of the time, what you will need will be there. 
...
After adding the chain to the `targetNetworks` array, you should add the chain to the `networks` object in your `hardhat.config.ts` file.
